...
import { AppRenderLayoutProps } from '../types/AppRenderProps';
...
import {
  ...
  SUPPORTED_LANGUAGES,
} from '../utils/i18n'; // XXX Init Sentry
import i18nextLocize, {
  fetchTranslations,
  I18nextResources,
} from '../utils/i18nextLocize';
...
const fileLabel = 'hoc/withLayout';
...
const withLayout = <P extends any>( // TODO: Add proper type
  ...
  class WithLayout extends React.Component<P> {
    ...
    render(): JSX.Element {
      ...
      const i18nextInstance = i18nextLocize(
        pageProps.lang,
        pageProps.defaultLocales
      ); // Apply i18next configuration with Locize backend
      ...
      if (layoutProps.isSSRReadyToRender) {
        ...
        if (isBrowser()) {
          ...
          amplitudeInstance.init(process.env.AMPLITUDE_API_KEY, null, {
            userId,
            logLevel:
              process.env.APP_STAGE === 'production' ? 'DISABLE' : 'WARN',
            includeGclid: true,
